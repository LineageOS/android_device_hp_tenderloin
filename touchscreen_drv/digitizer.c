...
try_again:
		...
		if (rc != 1)
			LOGE("TSPower, ioctl1 failed %d errno %d", rc, errno);
		...
		if ((rc != 1) && (retry_count++ < MAX_DIGITIZER_RETRY)) {
			...
			rc = write(vdd_fd, "0", 1);
			usleep(10000);
			LOGE("TS wakeup retry #%d", retry_count);
			goto try_again;
		}
        ...
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			LOGE("TSPower, ioctl2 failed %d errno %d", rc, errno);

        i2c_msg.len = 2;
        i2c_buf[0] = 0x30; i2c_buf[1] = 0x0F;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			LOGE("TSPower, ioctl3 failed %d errno %d", rc, errno);

        i2c_buf[0] = 0x40; i2c_buf[1] = 0x02;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			LOGE("TSPower, ioctl4 failed %d errno %d", rc, errno);

        i2c_buf[0] = 0x41; i2c_buf[1] = 0x10;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			LOGE("TSPower, ioctl5 failed %d errno %d", rc, errno);

        i2c_buf[0] = 0x0A; i2c_buf[1] = 0x04;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			LOGE("TSPower, ioctl6 failed %d errno %d", rc, errno);

        i2c_buf[0] = 0x08; i2c_buf[1] = 0x03;
        rc = ioctl(i2c_fd,I2C_RDWR,&i2c_ioctl_data);
		if (rc != 1)
			LOGE("TSPower, ioctl7 failed %d errno %d", rc, errno);
