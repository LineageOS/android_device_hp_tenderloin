	...
	for (k = 0; k < tpc; k++) {
		if (tpoint[k].isValid) {
			...
			if (smallest_delta > MAX_DELTA && previoustpc == tpc) {
				 ...
				 * ID and hope the userspace sorts it out properly which it
				 * appears to do so in testing - will use IDs the future */
...
void liftoff()
{
	...
	memset((char *)tpoint,255,sizeof(tpoint));
	memset((char *)prevtpoint,255,sizeof(prevtpoint));
	memset((char *)prev2tpoint,255,sizeof(prev2tpoint));
