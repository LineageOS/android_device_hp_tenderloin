...
void MPLSensor::setPowerStates(int enabled_sensors)
{
    ...
    if ((1 << Gyro) & enabled_sensors) {
        ALOGV(" Enabling gyroscope");
        mLocalSensorMask |= INV_THREE_AXIS_GYRO;
    } else {
        ALOGV(" Disabling gyroscope");
        mLocalSensorMask &= ~INV_THREE_AXIS_GYRO;
    }
...
int MPLSensor::enable(int32_t handle, int en)
{
    ...
    int newState = en ? 1 : 0;
    int err = 0;
    ALOGV_IF((uint32_t(newState) << Gyro) != (mEnabled & (1 << Gyro)),
            "sensor state change");

    pthread_mutex_lock(&mMplMutex);
    if ((uint32_t(newState) << Gyro) != (mEnabled & (1 << Gyro))) {
        uint32_t sensor_type;
        short flags = newState;
        mEnabled &= ~(1 << Gyro);
        mEnabled |= (uint32_t(flags) << Gyro);
