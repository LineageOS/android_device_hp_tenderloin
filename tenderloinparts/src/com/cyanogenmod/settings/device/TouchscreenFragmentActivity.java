...
public class TouchscreenFragmentActivity extends PreferenceFragment {
    ...
    private void syncPreferences() {
    	...
    	if (currentMode != null) {
    		...
    		if (currentMode.equalsIgnoreCase("F")) {
    			editor.putString( "touchscreen_mode_preference", values[0]);
    		} else if (currentMode.equalsIgnoreCase("S"))
    			editor.putString( "touchscreen_mode_preference", values[1]);
    		else {
    			Log.e(TAG, "Unable to get current touchscreen mode");
    		}
    ...
    private boolean setTouchscreenMode(String mode) {
		...
		try {
	    	...
	    	try {
	    		...
	    		Log.e(TAG, "unable to set current touchscreen mode. Result = " + result);
	    	}
	       	process.waitFor();
		} catch (IOException e) {
			e.printStackTrace();
